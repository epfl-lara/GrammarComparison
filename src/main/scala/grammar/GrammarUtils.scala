...
object GrammarUtils {
  ...
  def nullableFirstFollow[T](g: Grammar[T]) 
  	: (Set[Symbol[T]], Map[Symbol[T], Set[Terminal[T]]], Map[Symbol[T], Set[Terminal[T]]]) = {
    
    val nonTerminals = g.nonTerminals //g.rules.map(_.leftSide).distinct
    val terminals = g.terminals.toList //g.rules.flatMap(_.rightSide).distinct.collect { case t: Terminal => t }

    Util.fixpoint[(Set[Symbol[T]], Map[Symbol[T], Set[Terminal[T]]], Map[Symbol[T], Set[Terminal[T]]])] {
      case (nul: Set[Symbol[T]],
        fst: Map[Symbol[T], Set[Terminal[T]]],
        fol: Map[Symbol[T], Set[Terminal[T]]]) => {
        var nullable = nul
        var first = fst
        var follow = fol
      ...
      Map[Symbol[T], Set[Terminal[T]]]() ++ (for (X <- nonTerminals) yield (X: Symbol[T]) -> Set[Terminal[T]]()) ++ (for (Y <- terminals) yield (Y: Symbol[T]) -> Set[Terminal[T]](Y)),
      Map[Symbol[T], Set[Terminal[T]]]() ++ (for (X <- nonTerminals) yield (X: Symbol[T]) -> Set[Terminal[T]]())))
  }

  def firstA[T](x: List[Symbol[T]], nullable: Set[Symbol[T]], fst: Map[Symbol[T], Set[Terminal[T]]]): Set[Terminal[T]] = {
    x match {
      case Nil => Set.empty
      case (a: Nonterminal) :: b if !nullable(a) => fst(a)
      case (a: Terminal[T]) :: b => Set(a)
   ...
   */
  sealed abstract class LL1Feedback[+T]
  case class InLL1() extends LL1Feedback {
    override def toString = "The grammar is LL(1)"
  }

  case class IntersectingFirst[T](r1: Rule[T], r2: Rule[T], commonTerms: List[Terminal[T]]) extends LL1Feedback[T] {
    override def toString = {
      s"First($r1) \u2229 First($r2) = " + Util.setString(commonTerms)
    }
  }
  case class IntersectingFirstFollow[T](x: Nonterminal, commonTerms: List[Terminal[T]]) extends LL1Feedback[T] {
    override def toString = {
      s"$x is nullable and First($x) \u2229 Follow($x) = " + Util.setString(commonTerms)
    }
  }
  case class ManyNullables[T](r1: Rule[T], r2: Rule[T]) extends LL1Feedback[T] {
    override def toString = {
      val nt = r1.leftSide
  ...
  def isLL1WithFeedback[T](g: Grammar[T]): LL1Feedback[T] = {
